...
def run_single_training(arg_dict):
    ...
    p = subprocess.run("python3 RunSingleSplit_BL.py" + arg_str, shell=True, 
...
if __name__ == '__main__':
    ...
    PIPE_args = {
        "dataPath": ['/data/zanola/BrainLats_pickle/'],
        "pipelineToEval": ["icasr"],
        "taskToEval": ["alzheimerBL"],
        "modelToEval":  ["xeegnet", "eegnet", "shallownet", "deepconvnet", "fbcnet", "atcnet"], #["eegconf"],
        "downsample": [True],
        "z_score": [True],
        "rem_interp": [True],
        "batchsize": [64],
