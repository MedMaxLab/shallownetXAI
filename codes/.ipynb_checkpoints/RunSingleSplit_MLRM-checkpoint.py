...
if __name__ == '__main__':
    ...
    # Set the output path
    if taskToEval.casefold() == 'alzheimer':
        start_piece_res = 'AnalyticalCalculation/StatisticalModels/Results/'
        task_piece = 'alz'
    elif taskToEval.casefold() == 'cognitive':
        start_piece_res = 'AnalyticalCalculation/StatisticalModels/Results/'
        task_piece = 'cgn'
    ...
    f,Pxx = eegvislib.get_Pxx(test_window_normalized[0,:,:],srate=freq)
    ...
    for i in range(test_window_normalized.shape[0]):
        f,Pxx = eegvislib.get_Pxx(test_window_normalized[i,:,:],srate=freq)
    ...
    f,Pxx = eegvislib.get_Pxx(training_window_normalized[0,:,:],srate=freq)
    ...
    for i in range(training_window_normalized.shape[0]):
        f,Pxx = eegvislib.get_Pxx(training_window_normalized[i,:,:],srate=freq)
    ...
    result1 = model1.fit(method='ncg')
    # Predict on the test data
    predictions1 = result1.predict(X_test)
    predicted_classes1 = np.argmax(predictions1, axis=1)
    accuracy1 = balanced_accuracy_score(y_test, predicted_classes1)

    scores= {'MLRM': model,
             'MLRM_accuracy_weighted': accuracy,
             'shnMLRM': model1,
             'shnMLRM_accuracy_weighted': accuracy1}
